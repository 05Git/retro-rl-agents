...
class ConfigData:
    ...
    def get_sys_info(cls) -> str:
        ...
        if retcode == 0:
            ...
            warn_msg = (
                f"Cannot print sys info: 'nvidia-smi' return code: {retcode}."
            )
            warn(warn_msg)
            return warn_msg

    def get_tb_log_final_step_res(
        self,
        ...
        all_tb_logs = sorted(
            list(tb_log_path.glob(f"*{log_name}*")), key=lambda x: getmtime(x)
        )
        if not all_tb_logs:
            warn(
                f"No tb logs found which match given name {log_name}."
                " Returning 'None'."
            )
            return None, None
        ...
        if len(all_tb_logs) > 1:
            warn(
                "Found more than one tb log which match the given name"
        ...
        try:
            avg_return_final = final_values.loc[
                final_values["tag"] == "rollout/ep_rew_mean", "value"
            ].iloc[0]
            avg_ep_len_final = final_values.loc[
                final_values["tag"] == "rollout/ep_len_mean", "value"
            ].iloc[0]
        except IndexError:
            warn(
